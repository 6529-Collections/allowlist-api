  ...
  async searchContractMetadata(
    ...
    return response.map((contract) => {
      if (!contract.contractMetadata) {
        throw new Error('Invalid Alchemy contract metadata response');
      }
      return {
        address: contract.address,
        name: contract.contractMetadata.name,
        tokenType: this.normalizeTokenType(contract.contractMetadata.tokenType),
        openSea: this.normalizeOpenSeaMetadata(
          contract.contractMetadata.openSea ??
            contract.contractMetadata.opensea,
        ),
      };
    });
