...
export class PhaseComponentRepository {
  ...
  async getByPhaseId(param: {
    ...
    return this.db.many<PhaseComponentEntity>(
      `SELECT external_id as id, name, description, insertion_order, allowlist_id, phase_external_id as phase_id, wallets_count, tokens_count
    ...
    for (const entity of entities) {
      await this.db.none(
        `INSERT INTO phase_component (external_id, name, description, insertion_order, allowlist_id,
                                              phase_external_id, wallets_count, tokens_count)
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
        [
          entity.id,
          entity.name,
