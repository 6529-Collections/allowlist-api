...
export class PhaseComponentRepository {
  ...
  async getByPhaseId(param: {
    ...
    return this.db.many<PhaseComponentEntity>(
      `SELECT external_id as id, name, description, insertion_order, allowlist_id, phase_external_id as phase_id, wallets_count, tokens_count, winners_wallets_count, winners_spots_count
    ...
    for (const entity of entities) {
      await this.db.none(
        `INSERT INTO phase_component (external_id, name, description, insertion_order, allowlist_id,
                                              phase_external_id, wallets_count, tokens_count, winners_wallets_count, winners_spots_count)
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [
          entity.id,
          entity.name,
          ...
          entity.tokens_count,
          entity.winners_wallets_count,
          entity.winners_spots_count,
        ],
        options,
      );
    }
